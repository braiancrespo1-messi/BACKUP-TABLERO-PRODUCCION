...
async function selectClient(clientId, clientName) {
    ...
    try {
        ...
        // Fetch contacts for this client using children API (childId 38)
        try {
            const contactsUrl = `https://api.yiqi.com.ar/api/childrenApi/GetChildList?entityId=${CONFIG.ENTITY_CLIENTE}&schemaId=${CONFIG.SCHEMA_ID}&childId=38&instanceId=${clientId}`;
            const contactsRes = await apiCall(contactsUrl, "GET");
            console.log("GetChildList for contacts returned:", contactsRes);
            let contactsList = [];
            if (contactsRes) {
                ...
                } else if (Array.isArray(contactsRes.rows)) {
                    contactsList = contactsRes.rows;
                } else if (Array.isArray(contactsRes.instances)) {
                    contactsList = contactsRes.instances;
                }
            }
            selectedClient.contacts = contactsList;
        } catch (contactErr) {
            console.error("Error fetching client contacts:", contactErr);
        ...
        try {
            ...
            if (ccRes.ok) {
                ...
                selectedClient.balance = ccData.saldoActual !== undefined ? ccData.saldoActual : (ccData.totalDeudaPendiente !== undefined ? ccData.totalDeudaPendiente : initialBalance);
            }
        } catch (ccErr) {
...
function toggleClientContacts() {
    const detail = document.getElementById("client-contacts-detail");
    const arrow = document.querySelector(".collapsible-trigger .toggle-arrow-contacts");
    if (detail) {
        if (detail.style.display === "none" || !detail.style.display) {
            detail.style.display = "flex";
            if (arrow) arrow.style.transform = "rotate(90deg)";
...
window.toggleClientContacts = toggleClientContacts;
...
function renderClientCard() {
    ...
    if (contactCount > 0) {
        contactsHtml = `
            <div class="attr-row collapsible-trigger" onclick="toggleClientContacts()" style="cursor: pointer; user-select: none; margin-top: 4px;">
                <span class="attr-label" style="display: flex; align-items: center; gap: 4px; font-weight: 500;">
                    👥 Contactos
                    <span class="toggle-arrow-contacts" style="font-size: 0.7rem; transition: transform 0.2s; display: inline-block;">▶</span>
                </span>
                <span class="attr-value badge-value" style="background: rgba(16, 185, 129, 0.2); color: #10b981;">${contactCount}</span>
            </div>
            <div id="client-contacts-detail" class="collapsible-content" style="display: none; padding: 6px 12px; background: rgba(255, 255, 255, 0.02); border-left: 2px solid #10b981; margin: 4px 0 10px 0; border-radius: var(--radius-sm); font-size: 0.78rem; display: flex; flex-direction: column; gap: 6px;">
        `;
        contacts.forEach((c, idx) => {
            let roles = [];
            if (c.CONT_CONTACTO_FACTURACION === "S") roles.push(`<span style="background: rgba(74, 222, 128, 0.15); color: #4ade80; padding: 1px 4px; border-radius: 3px; font-size: 0.62rem; font-weight: 600; border: 1px solid rgba(74, 222, 128, 0.25);">Facturación</span>`);
            if (c.CONT_CONTACTO_COBRANZAS === "S") roles.push(`<span style="background: rgba(192, 132, 252, 0.15); color: #c084fc; padding: 1px 4px; border-radius: 3px; font-size: 0.62rem; font-weight: 600; border: 1px solid rgba(192, 132, 252, 0.25);">Cobranzas</span>`);
            
            contactsHtml += `
                <div style="padding: 4px 0; border-bottom: ${idx < contactCount - 1 ? '1px dashed rgba(255, 255, 255, 0.05)' : 'none'}; text-align: left; display: flex; flex-direction: column; gap: 2px;">
                    <div style="font-weight: 600; color: var(--text-primary);">${c.CONT_NOMBRE || 'Sin nombre'} ${c.CONT_PUESTO ? '<span style="color: var(--text-secondary); font-weight: normal; font-size: 0.7rem;">(' + c.CONT_PUESTO + ')</span>' : ''}</div>
                    ${c.CONT_EMAIL ? '<div style="color: var(--text-secondary); font-size: 0.72rem;">✉️ ' + c.CONT_EMAIL + '</div>' : ''}
                    ${roles.length > 0 ? '<div style="display: flex; gap: 4px; margin-top: 2px;">' + roles.join(" ") + '</div>' : ''}
                </div>
            `;
        });
        contactsHtml += `
        ...
        contactsHtml = `
            <div class="attr-row">
                <span class="attr-label">👥 Contactos</span>
                <span class="attr-value" style="opacity: 0.5;">0</span>
            </div>
        `;
    }
...
function updateCreatorClientWidget() {
    ...
    const headerToggle = document.getElementById("doc-header-toggle");
    ...
    if (window.docHeaderExpanded === undefined) {
        window.docHeaderExpanded = true;
    }
    
    if (window.docHeaderExpanded) {
        branchContainer.style.display = "flex";
        const toggleIcon = document.getElementById("doc-header-toggle-icon");
...
function createCrmFollowupObject(clientId, clientName, docId, docType, docTotal, docDisplayNum, dateCreated = null, clientPhone = null) {
    ...
    const sellerName = (selectedClient && String(selectedClient.id) === String(clientId)) ? selectedClient.seller : "Fábrica";
    return {
...
function shareViaWhatsApp(planId, stepIndex, phone) {
    ...
    let sellerName = plan ? plan.vendedor : null;
...
function renderConversationsList() {
    ...
    const searchQuery = (document.getElementById("inbox-search-input")?.value || "").trim().toLowerCase();
    
    // Filter chats based on filterSeller and searchQuery
    const filteredChats = chatsList.filter(chat => {
        // 1. Seller filter
        if (filterSeller) {
            const assignedSeller = (chat.assignedSeller || "").trim().toLowerCase();
            const targetSeller = filterSeller.trim().toLowerCase();
            if (assignedSeller !== targetSeller) return false;
        }
        
        // 2. Search query filter
        if (searchQuery) {
            const phone = (chat.phone || "").toLowerCase();
            const clientName = (chat.clientName || "").toLowerCase();
            const contactName = (chat.contactName || "").toLowerCase();
            const name = (chat.name || "").toLowerCase();
            
            const matchPhone = phone.includes(searchQuery);
            const matchClientName = clientName.includes(searchQuery);
            const matchContactName = contactName.includes(searchQuery);
            const matchName = name.includes(searchQuery);
            
            if (!matchPhone && !matchClientName && !matchContactName && !matchName) {
                return false;
            }
        }
...
// Toggles visibility of the document header settings (Sucursal, Condición de Pago, Método de Envío)
function toggleDocHeader() {
    const container = document.getElementById("doc-sucursal-container");
    const toggleIcon = document.getElementById("doc-header-toggle-icon");
    ...
    if (container.style.display === "none") {
        ...
        toggleIcon.textContent = "▼";
        window.docHeaderExpanded = false;
    }
}
window.toggleDocHeader = toggleDocHeader;

// Toggles visibility of the totals detailed breakdown (Subtotal, IVA, Descuento Global) expanding upwards
function toggleTotalsDetails(event) {
    if (event.target.tagName === "INPUT" || event.target.tagName === "BUTTON" || event.target.tagName === "SELECT") {
        return;
    }
    
    const details = document.getElementById("totals-details");
    const arrow = document.getElementById("totals-toggle-arrow");
...
window.toggleTotalsDetails = toggleTotalsDetails;
