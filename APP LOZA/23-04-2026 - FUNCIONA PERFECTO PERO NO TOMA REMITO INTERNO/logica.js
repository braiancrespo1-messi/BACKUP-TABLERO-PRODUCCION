...
const DEFAULT_CONFIG = {
    ...
    smartieNroRemitoExterno: 2752, // [NUEVO] SMARTIE DE NUMERACIONES EXTERNAS (ENTITY 781)
    valorDeclarado: 1000,
    slaThresholdDays: 15,
...
const REMITO_SEQ_KEY = 'fabrica_remito_seq';

function getNextRemitoSeq() {
    let seq = parseInt(localStorage.getItem(REMITO_SEQ_KEY) || '0');
    seq++;
    localStorage.setItem(REMITO_SEQ_KEY, String(seq));
    return seq;
}

function getCurrentRemitoSeq() {
    return parseInt(localStorage.getItem(REMITO_SEQ_KEY) || '0');
}

/**
 * Sincroniza el contador local con el último Nro Remito Externo de YiQi.
 * La smartie 2752 está ordenada DESC, así que el primer registro es el más alto.
 */
async function initRemitoSeq() {
    let yiqiMax = 0;
    try {
        console.log('📋 Consultando último Nro Remito Externo en YiQi (Smartie 2752)...');
    ...
    const localSeq = parseInt(localStorage.getItem(REMITO_SEQ_KEY) || '0');

    if (yiqiMax > 0) {
        if (yiqiMax > localSeq) {
            console.log(`📋 Actualizando secuencia local: ${localSeq} → ${yiqiMax} (sincronizado con YiQi)`);
            localStorage.setItem(REMITO_SEQ_KEY, String(yiqiMax));
        }
    } else if (localSeq === 0) {
        // Fallback manual si no hay datos
        const startStr = await showPrompt(
            "Configurar Numeración",
            "Ingrese el <b>último número de remito externo</b> utilizado.<br><small>Ej: si el último fue 1705, ingrese 1705.</small>",
            "0", "number"
        );
        const startNum = parseInt(startStr) || 0;
        localStorage.setItem(REMITO_SEQ_KEY, String(startNum));
    }
}
