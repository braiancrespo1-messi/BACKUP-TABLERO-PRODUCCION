...
async function run() {
  try {
    ...
    const ids = [10458, 20073, 11030];
    for (const id of ids) {
      const data = await getDetails(id, token);
      console.log(`=== ID: ${id} (${data.name || "Sin Nombre"}) ===`);
      console.log("currentState:", data.currentState);
      console.log("actions:", data.actions ? data.actions.map(a => a.name) : []);
      console.log("transitions:", data.transitions ? data.transitions.map(t => t.name) : []);
    }
  } catch (e) {
    console.error(e);
  }
