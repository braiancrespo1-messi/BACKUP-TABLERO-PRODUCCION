...
/**
 * Realiza consultas paginadas de forma secuencial al endpoint GetList de YiQi ERP
 * para evitar el límite/truncamiento duro de 50 registros por página.
 */
async function fetchAllInstances(url, headers, bodyTemplate) {
  let allRows = [];
  let page = 1;
  const pageSize = 50; // Límite máximo admitido por GetList
  let hasMore = true;

  while (hasMore) {
    const body = { ...bodyTemplate, page, pageSize };
    ...
    if (!resp.ok) {
      throw new Error(`YiQi GetList failed with HTTP ${resp.status}`);
    }
    ...
    const totalCount = data.totalCount || data.total || data.count || 0;
    
    // Detener si recibimos menos filas del límite (última página) o si alcanzamos el totalCount
    if (rows.length < pageSize || allRows.length >= totalCount) {
      hasMore = false;
    } else {
      page++;
    }
...
exports.controlCalidadApi = onRequest({ cors: true }, async (req, res) => {
  ...
  try {
    ...
    if (action === "getTableroPedidos") {
      ...
      if (uniqueSkus.length > 0) {
        try {
          await Promise.all(uniqueSkus.map(async (sku) => {
            ...
            if (bomResp.ok) {
              const bomData = await bomResp.json();
              const bomRows = bomData.data || bomData.rows || bomData.instances || bomData.items || [];
              const matched = bomRows.some(r => (r.MBOM_CODIGO || "").trim().toUpperCase() === String(sku).trim().toUpperCase());
              if (matched) {
                bomMap[sku.trim().toUpperCase()] = true;
              }
            }
          }));
        } catch (bomErr) {
      ...
      // Pedidos a preparar (Paginación protegida)
      ...
      for (let i = 1; i <= 12; i++) {
        ...
        try {
          const rows = await fetchAllInstances(getListRemitosUrl, headers, {});
          ...
          console.error(`Error al buscar remitos paginados (intento ${i}):`, err);
        }
        await sleep(2000);
      }
      ...
      // Jaulas pendientes de control (Paginación protegida)
      ...
      try {
        ...
        console.error("Error al obtener jaulas pendientes:", err);
        return res.status(500).json({ error: err.message });
      }
...
exports.guardarDatosCrm = onRequest({ cors: true }, async (req, res) => {
  ...
  try {
    ...
    if (action === "saveFollowup") {
      ...
      // Write normalized key
      await db.collection("crm_whatsapp_configs").doc(sellerKey).set(docPayload, { merge: true });
      
      // Write accented legacy key if FABRICA/FÁBRICA to ensure backward compatibility
      if (sellerKey === "FABRICA") {
        await db.collection("crm_whatsapp_configs").doc("FÁBRICA").set(docPayload, { merge: true });
      }
...
exports.consultarCuitAfip = onRequest({ cors: true }, async (req, res) => {
  ...
  const ignoreYiqi = req.query.ignoreYiqi === "true" || req.body.ignoreYiqi === true || req.query.force === "true" || req.body.force === true;

  // Verificar si ya existe en YiQi antes de buscar en AFIP
  if (!ignoreYiqi) {
    try {
    const token = await getYiQiToken();
    ...
    if (queryResp.ok) {
      ...
      const matched = existingList.filter(row => String(row.CLIE_CUIT || "").replace(/\D/g, "") === cuit);
      
      if (matched.length > 0) {
        console.log(`[AFIP] El CUIT ${cuit} ya existe en YiQi. Retornando ${matched.length} coincidencias.`);
        return res.json({
          success: true,
          existsInYiqi: true,
          matches: matched.map(m => ({
            id: m.id || m.ID,
            nombre: m.CLIE_NOMBRE || "",
            razonSocial: m.CLIE_RAZON_SOCIAL || "",
            ivaCondition: m.COIV_NOMBRE || m.COIV_DESCRIPCION || "S/D"
          }))
        });
      }
    }
  } catch (err) {
    console.error("[AFIP] Error checking duplicate CUIT in YiQi:", err);
  }
  }
  ...
  let lastError = "No se encontraron datos impositivos en AFIP para la CUIT ingresada.";
  ...
  if (sdkToken) {
    ...
    try {
      ...
      if (details && details.errorConstancia) {
        const errors = details.errorConstancia.error || [];
        lastError = "AFIP: " + (Array.isArray(errors) ? errors.join(" / ") : String(errors));
      }
      if (details && !details.errorConstancia) {
        ...
        if (!socialName) {
          throw new Error("No se obtuvo la Razón Social del padrón AFIP.");
        }
      ...
      lastError = "AFIP SDK: " + (err.message || String(err));
    }
  }
  ...
  if (API_KEY) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        lastError = `Cuitalizer API retornó HTTP ${resp.status}`;
      }
    } catch (err) {
      console.error(`[AFIP] Error consultando Cuitalizer API:`, err);
      lastError = `Cuitalizer: ${err.message || String(err)}`;
    }
  }

  console.log(`[AFIP] Consulta fallida. Retornando error: ${lastError}`);
...
exports.enviarWhatsApp = onRequest({ cors: true }, async (req, res) => {
  try {
    ...
    if (sellerName) {
      ...
      let doc = await db.collection("crm_whatsapp_configs").doc(sellerKey).get();
      if (!doc.exists) {
        // Fallback for accented legacy documents
        const legacyKey = String(sellerName).trim().toUpperCase();
        if (legacyKey !== sellerKey) {
          doc = await db.collection("crm_whatsapp_configs").doc(legacyKey).get();
        }
      }
      if (doc.exists) {
        const data = doc.data();
    ...
    if (!apiUrl) {
      let doc = await db.collection("crm_whatsapp_configs").doc("FABRICA").get();
      if (!doc.exists) {
        // Fallback for legacy FÁBRICA accented document
        doc = await db.collection("crm_whatsapp_configs").doc("FÁBRICA").get();
      }
      if (doc.exists) {
        const data = doc.data();
        apiUrl = data.apiUrl;
...
/**
 * Cloud Function que busca proveedores en YiQi ERP (Smartie 2603, entityId 345).
 */
exports.buscarProveedoresProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const searchTerm = req.query.search || req.body.search || "";
    const page = req.query.page ? parseInt(req.query.page, 10) : 1;
    const pageSize = req.query.pageSize ? parseInt(req.query.pageSize, 10) : 50;

    console.log(`[Proxy] Buscando proveedores activos con término: "${searchTerm}"`);
    const token = await getYiQiToken();

    const url = "https://api.yiqi.com.ar/api/public/CLIENTE/query?schemaId=1491";
    const queryBody = {
      page: page,
      pageSize: pageSize,
      columns: [
    ...
    if (searchTerm) {
      queryBody.search = String(searchTerm);
    }
    ...
    if (!response.ok) {
      const errText = await response.text();
      return res.status(response.status).json({ success: false, error: `Error de YiQi: ${errText}` });
    }

    const data = await response.json();
    const rows = data.data || data.rows || data.instances || [];

    const suppliers = rows.map(r => ({
      id: r.ID || r.id,
      codigo: r.CLIE_CODIGO || "",
      nombre: r.CLIE_NOMBRE || "",
      razonSocial: r.CLIE_RAZON_SOCIAL || "",
      cuit: r.CLIE_CUIT || "",
    ...
    console.error("Error en buscarProveedoresProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que actúa como proxy para registrar una Factura de Compra en YiQi ERP.
 * Crea la cabecera (Entity 660), guarda los ítems de detalle (Child 140) y opcionalmente ejecuta transiciones de workflow.
 */
exports.crearFacturaCompraProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const {
      supplierId,        // id del proveedor (ej. 6227)
      invoiceType,       // 1: A, 2: B, 3: C, 17: X (por defecto 17)
      pos,               // Punto de venta (ej. "99")
      number,            // Número de comprobante (ej. "20062026")
      dateEmission,      // Fecha emisión (DDMMYYYY o ISO string)
      dateDue,           // Fecha vencimiento (ISO string o YYYY-MM-DD)
      currencyId,        // Moneda (171: ARS, 172: USD)
      exchangeRate,      // Cotización (por defecto 1)
      rubroId,           // Rubro/Imputación (1: Productos, 2: Servicios, 5: Bienes de uso)
    ...
    if (!supplierId || !pos || !number || !items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ success: false, error: "Faltan parámetros requeridos (supplierId, pos, number, items)" });
    }
    ...
    const formParams = [
      ...
      `6893=${exchangeRate || 1}`,
      `9435=${rubroId || 5}`,
      `9058=${jurisdiccionId || 2}`,
      `2881=${encodeURIComponent(dateEmission || "")}`,
      `2886=${encodeURIComponent(dateDue || "")}`,
      `2889=`,
      `2890=`,
      `2887=`,
      `2885=`,
    ...
    console.log("[Proxy] Guardando cabecera de Factura de Compra...");
    const saveResp = await fetch(saveUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!saveResp.ok) {
      const errText = await saveResp.text();
      throw new Error(`Error de YiQi al guardar cabecera: HTTP ${saveResp.status} - ${errText}`);
    }

    const saveData = await saveResp.json();
    if (saveData.ok === false || (!saveData.newId && !saveData.id && !(saveData.ok && saveData.data && saveData.data.id))) {
      const errMsg = saveData.error || saveData.message || "No se pudo obtener el ID del comprobante generado por YiQi ERP.";
      throw new Error(errMsg);
    }
    const invoiceId = saveData.newId || saveData.id || (saveData.data ? saveData.data.id : null);

    console.log(`[Proxy] Cabecera creada. ID Factura: ${invoiceId}. Guardando ítems...`);

    const childUrl = `https://api.yiqi.com.ar/api/childrenApi/SaveChildInstances?instanceId=${invoiceId}&schemaId=1491`;
    const childInstancesPayload = items.map(item => {
      return JSON.stringify({
        id: "",
        CODC_ID_CODC: null,
        CONCEPTO: item.concepto || "Detalle Factura",
    ...
    const childResp = await fetch(childUrl, {
      ...
      body: JSON.stringify({
        ...
        childInstances: childInstancesPayload,
        append: false
      })
    });

    if (!childResp.ok) {
      const errText = await childResp.text();
      throw new Error(`Error de YiQi al guardar conceptos del detalle: HTTP ${childResp.status} - ${errText}`);
    }
    ...
    if (autoTransition) {
      console.log(`[Proxy] Ejecutando transición ingresar (118128) para factura ${invoiceId}...`);
      const transUrl = "https://api.yiqi.com.ar/api/workflowApi/ExecuteTransition";

      let transitionForm = `2881=${encodeURIComponent(dateEmission || "")}`;

      const trans1Resp = await fetch(transUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      if (!trans1Resp.ok) {
        console.error(`[Proxy] Error en transición 118128: HTTP ${trans1Resp.status}`);
      } else {
        console.log("[Proxy] Transición 118128 ejecutada con éxito.");
      }
    }

    return res.json({ success: true, invoiceId: invoiceId });

  } catch (error) {
    console.error("Error en crearFacturaCompraProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que actúa como Proxy Seguro para el Alta/Actualización de Proveedores en YiQi ERP.
 * Valida duplicados, guarda la cabecera del proveedor (Entity 345, con 5772=on).
 */
exports.altaProveedorProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const {
      socialName,
      fantasyName,
    ...
    if (!socialName || !address || !city || !provinceId || !zip) {
      return res.status(400).json({ success: false, error: "Faltan parametros requeridos para el alta (socialName, address, city, provinceId, zip)" });
    }

    const cuitClean = cuit ? String(cuit).replace(/\D/g, "") : "";
    if (cuitClean && cuitClean.length !== 11 && cuitClean.length !== 7 && cuitClean.length !== 8) {
      return res.status(400).json({ success: false, error: "El documento debe tener 7, 8 u 11 digitos numericos" });
    }

    const docType = req.body.documentType || (cuitClean ? (cuitClean.length === 11 ? 1 : 3) : 3);
    const docTypeLabel = docType === 3 ? "DNI" : "CUIT";

    console.log(`[ProxyProveedor] Iniciando alta de proveedor nuevo para ${docTypeLabel}: ${cuitClean} - Razon Social: ${socialName}`);

    // 1. Obtener Token de YiQi ERP
    const token = await getYiQiToken();

    // 2. Crear Empresa (Entity 345) en YiQi ERP (siempre nuevo registro, no actualizar existentes)
    console.log(`[ProxyProveedor] Guardando cabecera de proveedor nuevo en YiQi...`);
    const formStr = [
      `2468=${encodeURIComponent(socialName)}`,
      `948=${encodeURIComponent(fantasyName || socialName)}`,
      `5681=${docType}`,
      ...
      `3821=${ivaCondition || 7}`,
      `1268=on`,
      `5772=on`, // SUPPLIER flag on
      `9971=off`,
      `5782=${encodeURIComponent(iibb || cuitClean)}`,
      `6671=${encodeURIComponent(inicioActividades || "")}`,
      `3148=${parseInt(subconceptoGanancias, 10) || 78}`
    ].join('&');
    ...
    if (!saveResp.ok) {
      const errText = await saveResp.text();
      throw new Error(`Error al registrar el proveedor en YiQi: HTTP ${saveResp.status} - ${errText}`);
    }

    const saveData = await saveResp.json();
    const clientId = saveData.newId || saveData.id || (saveData.ok && saveData.data ? saveData.data.id : null);
    if (!clientId) {
      throw new Error("No se pudo obtener el ID del proveedor generado por YiQi ERP.");
    }

    console.log(`[ProxyProveedor] Proveedor guardado con éxito. ID ERP: ${clientId}`);
    ...
    console.error("Error en altaProveedorProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que actúa como proxy para obtener el catálogo de Conceptos de Perrupción de YiQi ERP.
 */
exports.obtenerPercepcionesProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = await getYiQiToken();
    const url = "https://api.yiqi.com.ar/api/instancesApi/GetList?entityId=690&schemaId=1491&smartieId=2806";
    const headers = {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`
    };
    const allRows = await fetchAllInstances(url, headers, { search: "" });
    return res.json({ success: true, data: allRows });
  } catch (error) {
    console.error("Error en obtenerPercepcionesProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que consulta las reglas OCR guardadas en Firestore.
 * Puede filtrar por cuit o devolver todas las reglas de la colección.
 */
exports.obtenerReglasOcrProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    ...
    if (cuit) {
      const cuitClean = String(cuit).replace(/\D/g, "");
      const doc = await db.collection("ocr_supplier_layouts").doc(cuitClean).get();
      if (!doc.exists) {
        return res.json({ success: true, exists: false, data: null });
      }
      return res.json({ success: true, exists: true, data: doc.data() });
    } else {
      const snap = await db.collection("ocr_supplier_layouts").get();
      const list = [];
      snap.forEach(doc => {
        list.push(doc.data());
      });
    ...
    console.error("Error en obtenerReglasOcrProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que guarda o actualiza una regla de lectura OCR para un proveedor (CUIT).
 */
exports.guardarReglaOcrProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const { cuitProveedor, nombreProveedor, reglas } = req.body || {};
    if (!cuitProveedor) {
      return res.status(400).json({ success: false, error: "Falta parámetro requerido: cuitProveedor" });
    }

    const cuitClean = String(cuitProveedor).replace(/\D/g, "");
    const db = admin.firestore();

    const payload = {
      cuitProveedor: cuitClean,
      nombreProveedor: nombreProveedor || "Proveedor OCR",
      lastUpdated: new Date().toISOString(),
      reglas: reglas || {}
    };

    await db.collection("ocr_supplier_layouts").doc(cuitClean).set(payload, { merge: true });
    return res.json({ success: true, message: `Regla OCR para CUIT ${cuitClean} guardada correctamente.` });
  } catch (error) {
    console.error("Error en guardarReglaOcrProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});

/**
 * Cloud Function que elimina una regla OCR de un proveedor en Firestore.
 */
exports.eliminarReglaOcrProxy = onRequest({ cors: true }, async (req, res) => {
  try {
    const { cuitProveedor } = req.body || req.query || {};
    if (!cuitProveedor) {
      return res.status(400).json({ success: false, error: "Falta parámetro requerido: cuitProveedor" });
    }
    ...
    await db.collection("ocr_supplier_layouts").doc(cuitClean).delete();
    return res.json({ success: true, message: `Regla OCR para CUIT ${cuitClean} eliminada correctamente.` });
  } catch (error) {
    console.error("Error en eliminarReglaOcrProxy:", error);
    return res.status(500).json({ success: false, error: error.message });
  }
});
